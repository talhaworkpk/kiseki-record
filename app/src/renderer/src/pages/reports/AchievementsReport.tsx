...
export default function AchievementsReport() {
 ...
 <div className="bg-card border border-border shadow-sm p-6 rounded-2xl mb-8 flex items-center gap-6">
 ...
 {milestones.map(m => (
 <div key={m.id} className={`p-6 rounded-2xl border transition-all ${m.unlocked ? 'bg-card border-border shadow-sm hover:border-yellow-500/50' : 'bg-background border-dashed border-border shadow-sm opacity-70 grayscale'}`}>
