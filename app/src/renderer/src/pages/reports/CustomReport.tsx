...
import { FileSearch, Check, Printer, FileText, Download, X } from 'lucide-react'
...
export default function CustomReport() {
 ...
 const createdDate = new Date(userProfile.createdAt).toISOString().split('T')[0]
 setStartDate(createdDate)
 } else {
 const now = new Date()
 const start = new Date(now.getFullYear(), 0, 1) // default to start of year if no profile
 setStartDate(start.toISOString().split('T')[0])
 }
 setEndDate(new Date().toISOString().split('T')[0])
 } catch (err) {
 console.error('Failed to fetch initial dates', err)
 }
 }
 fetchInitialDates()
 }, [])
 ...
 {/* Configuration Form */}
 <div className="lg:col-span-1 bg-card border border-border shadow-sm p-6 rounded-2xl h-fit">
 <h3 className="font-bold mb-4">Report Type</h3>
 <select 
 value={reportType} 
 onChange={(e) => handleReportTypeChange(e.target.value)}
 className="w-full p-2 mb-6 bg-background border border-border shadow-sm rounded-md"
 >
 ...
 <input type="date" value={startDate} onChange={e => {setStartDate(e.target.value); setReportType('Custom')}} className="w-full p-2 bg-background border border-border shadow-sm rounded-md" />
 </div>
 <div>
 <label className="block text-sm font-medium mb-1">End Date</label>
 <input type="date" value={endDate} onChange={e => {setEndDate(e.target.value); setReportType('Custom')}} className="w-full p-2 bg-background border border-border shadow-sm rounded-md" />
 </div>
 </div>
 ...
 <label key={key} onClick={() => handleToggle(key as keyof typeof categories)} className="flex items-center gap-3 p-2 rounded-md hover:bg-accent cursor-pointer border border-transparent hover:border-border shadow-sm transition-colors">
 <div className={`w-5 h-5 rounded flex items-center justify-center border ${categories[key as keyof typeof categories] ? 'bg-primary border-primary text-primary-foreground' : 'bg-background border-border shadow-sm text-transparent'}`}>
 <Check size={14} />
 </div>
 ...
 <button onClick={handlePrint} disabled={exporting} className="p-2 bg-background border border-border shadow-sm rounded-md hover:bg-accent text-foreground transition-colors disabled:opacity-50" title="Print to PDF">
 <Printer size={18}/>
 </button>
 <button className="p-2 bg-background border border-border shadow-sm rounded-md hover:bg-accent text-foreground transition-colors" title="Export Raw JSON">
 <Download size={18}/>
 </button>
 </div>
 ...
 <div className="bg-card border border-dashed border-border shadow-sm rounded-2xl h-full flex flex-col items-center justify-center p-12 text-muted-foreground text-center">
 ...
 <div className="bg-background rounded-lg p-6 max-w-5xl w-full mx-4 h-[90vh] flex flex-col">
 ...
 <button onClick={() => setPdfZoom(Math.max(0.5, pdfZoom - 0.25))} className="px-3 py-1 border border-border shadow-sm rounded-md hover:bg-accent text-sm" disabled={pdfZoom <= 0.5}>Zoom Out</button>
 <span className="text-sm font-medium w-16 text-center">{Math.round(pdfZoom * 100)}%</span>
 <button onClick={() => setPdfZoom(Math.min(2, pdfZoom + 0.25))} className="px-3 py-1 border border-border shadow-sm rounded-md hover:bg-accent text-sm" disabled={pdfZoom >= 2}>Zoom In</button>
 <button onClick={() => setPdfZoom(1)} className="px-3 py-1 border border-border shadow-sm rounded-md hover:bg-accent text-sm">Reset</button>
 </div>
 ...
 <button onClick={handleClosePreview} className="px-4 py-2 border border-border shadow-sm rounded-md hover:bg-accent">Back</button>
